...
def filenameModeCallback(n):
	global filenameMode
	buttons[6][filenameMode + 3].setBg('radio3-0')
	filenameMode = n
	buttons[6][filenameMode + 3].setBg('radio3-1')

def timelapseModeCallback(n):
	global timelapseMode
	buttons[7][timelapseMode + 3].setBg('radio3-0')
	timelapseMode = n
	buttons[7][timelapseMode + 3].setBg('radio3-1')
	#setTimelapseMode()
...
buttons = [
  ...
  [Button((  0,188,320, 52), bg='done', cb=doneCallback),
   ...
   Button(( 56, 60,100,120), bg='radio3-1', fg='store-folder', cb=storeModeCallback, value=0),
   #Button((164, 60,100,120), bg='radio3-0', fg='store-boot', cb=storeModeCallback, value=1),
  ...
  # Screen mode 6 is filename
  [Button((  0,188,320, 52), bg='done', cb=doneCallback),
   Button((  0,  0, 80, 52), bg='prev', cb=settingCallback, value=-1),
   Button((240,  0, 80, 52), bg='next', cb=settingCallback, value= 1),
   Button((  2, 60,100,120), bg='radio3-1', fg='filename-epoch', cb=filenameModeCallback, value=0),
   Button((110, 60,100,120), bg='radio3-0', fg='filename-gmt1', cb=filenameModeCallback, value=1),
   Button((218, 60,100,120), bg='radio3-0', fg='filename-img', cb=filenameModeCallback, value=2),
   Button((  0, 10,320, 29), bg='filename')],

  # Screen mode 7 is timelapse
  [Button((  0,188,320, 52), bg='done', cb=doneCallback),
   Button((  0,  0, 80, 52), bg='prev', cb=settingCallback, value=-1),
   Button((240,  0, 80, 52), bg='next', cb=settingCallback, value= 1),
   Button((  2, 60,100,120), bg='radio3-1', fg='timelapse-20sec', cb=timelapseModeCallback, value=0),
   Button((110, 60,100,120), bg='radio3-0', fg='timelapse-1min', cb=timelapseModeCallback, value=1),
   Button((218, 60,100,120), bg='radio3-0', fg='timelapse-5min', cb=timelapseModeCallback, value=2),
   Button((  0, 10,320, 29), bg='timelapse')],
...
def setIsoMode(n):
	...
	buttons[8][5].setBg('iso-' + str(isoData[isoMode][0]))
	buttons[8][7].rect = ((isoData[isoMode][1] - 10,) +
	  buttons[8][7].rect[1:])

#def setTimelapseMode():
#	#takePicture()
#	print 'test'
...
def loadSettings():
	try:
	  ...
	  if 'tl'    in d: timelapseModeCallback( d['tl'])
	  if 'fn'    in d: filenameModeCallback( d['fn'])
...
#	  saveIdx += 1
#	  if saveIdx > 9999: saveIdx = 0
	filename = '{}/{}.jpg'.format(pathData[storeMode], timestamp)
...
while(True):
  ...
  if timelapseMode == 0: #20 seconds
    waitSeconds = 20
  elif timelapseMode == 1: #1 minute
    waitSeconds = 60
  elif timelapseMode == 2: #5 minutes
    waitSeconds = 300
  else:
    waitSeconds = 60

  if int(time.time()) > (timestamp + waitSeconds):
    timestamp = int(time.time())
    print 'taking picture timelapse picture {}'.format(timestamp)
    takePicture()
